...
Gemini APIの無料枠を利用し、2つのタブを切り替えて記事を作成できるStreamlitアプリです。
...
メイン画面上部に「キーワードからSEO記事」「URLから紹介記事」の2つのタブがあります。APIキーとモデルの設定は左側のサイドバーに分離しています。生成後は「想定読者の悩み」「記事の構成案」「完成した本文」「WordPress貼り付け用HTML」を分けて表示し、Markdown形式とHTML形式で保存できます。
